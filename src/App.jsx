...
const css = `
  ...
  :root{--bg:#090910;--surface:#111119;--surface2:#181824;--border:rgba(255,255,255,0.07);--text:#EEEEF8;--muted:rgba(238,238,248,0.42);--accent:#FF4D00;--accent2:#FF8C00;--blue:#00C2FF;--green:#00E676;--yellow:#FFD600;--sidebar:220px;--radius:14px;--font-display:'Bebas Neue',sans-serif;--font-body:'DM Sans',sans-serif}
...
function LiveMap({ currentUser, running, onTrailUpdate, savedTrails }) {
  ...
  useEffect(() => {
    ...
    savedTrails.forEach(trail => {
      if (trail.points?.length > 1) L.polyline(trail.points, { color: trail.color, weight: 5, opacity: 0.7 }).addTo(map);
    });
  }, [savedTrails]);
      ...
      <div className="map-overlay-top">
        ...
        {savedTrails.slice(0, 5).map((t, i) => (
          <div key={i} className="legend-item">
            ...
            <span style={{ color: 'var(--muted)', marginLeft: 'auto', fontSize: 11 }}>{Number(t.km || 0).toFixed(1)}km</span>
          </div>
        ))}
      </div>
...
export default function App() {
  ...
  const loadTrails = async () => {
    const data = await sb.from('trails').select('*', { order: 'created_at.desc', limit: 50 });
    ...
    <>
      ...
      <div className="app">
        <aside className="sidebar">
          ...
          {[{ id:'map',icon:'map',label:'Mapa'},{ id:'ranking',icon:'trophy',label:'Ranking'},{ id:'discuss',icon:'chat',label:'Discussão'},{ id:'profile',icon:'user',label:'Perfil'},...(currentUser?.is_admin?[{id:'admin',icon:'shield',label:'Admin'}]:[])].map(item => (
            <button key={item.id} className={`nav-item${page===item.id?' active':''}`} onClick={() => setPage(item.id)}><Icon name={item.icon} size={18}/>{item.label}</button>
          ))}
          <div className="sidebar-bottom">
            ...
            <button className="nav-item" style={{marginTop:6}} onClick={() => {setCurrentUser(null);setScreen('landing');setAuthMode(null);setRunning(false);setPosts([]);setUsers([]);setSavedTrails([]);}}><Icon name="logout" size={18}/>Sair</button>
          </div>
        </aside>
        <main className="main">
          {page==='map' && (<>
              ...
              {filteredPosts.map(post => (
                <div key={post.id} className="post-card">
                  ...
                  <div className="post-footer">
                    ...
                    <button className={`post-action-btn${openComments.has(post.id)?' commented':''}`} onClick={()=>toggleComments(post.id)}><Icon name="comment" size={15}/>{(post.comments||[]).length} comentário{(post.comments||[]).length!==1?'s':''}</button>
                  </div>
                  {openComments.has(post.id) && (
                    <div className="comments-section">
                      ...
                      <div className="comment-input-row">
                        ...
                        <input className="comment-input" placeholder="Escreva um comentário..." value={commentTexts[post.id]||''} onChange={e=>setCommentTexts(prev=>({...prev,[post.id]:e.target.value}))} onKeyDown={e=>e.key==='Enter'&&handleComment(post.id)}/>
          ...
          {page==='profile' && (
            <div className="page">
              ...
              <div className="profile-stats">
                <div className="stat-card"><div className="stat-card-value" style={{color:currentUser.color}}>{Number(currentUser.total_distance||0).toFixed(1)}</div><div className="stat-card-label">km percorridos</div></div>
                ...
                <div className="stat-card"><div className="stat-card-value" style={{color:currentUser.color}}>{Number(currentUser.dominated_area||0).toFixed(2)}</div><div className="stat-card-label">km² de área</div></div>
              </div>
              <div className="card">
              ...
              <div className="admin-section">
                ...
                <div className="card" style={{padding:0,overflow:'hidden'}}>
                  <table className="admin-table">
                    <thead><tr><th>Cor</th><th>Nome</th><th>Email</th><th>Cidade</th><th>km Total</th><th>km Dom.</th><th>Tipo</th></tr></thead>
              ...
              <div className="admin-section">
                ...
                <div className="card" style={{padding:0,overflow:'hidden'}}>
                  <table className="admin-table">
                    ...
                    <tbody>{posts.map(p=><tr key={p.id}><td style={{fontWeight:600}}>{p.user_name}</td><td>{p.city}</td><td style={{maxWidth:200,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',color:'var(--muted)',fontSize:12}}>{p.text}</td><td>{(p.comments||[]).length}</td><td>{p.likes||0}</td></tr>)}</tbody>
                  </table>
                </div>
              </div>
            </div>
        ...
        <div className="modal-overlay">
          <div className="modal" style={{maxWidth:380}}>
            <div className="modal-header">
              <div><div className="modal-title">ATIVIDADE CONCLUÍDA</div><div style={{fontSize:12,color:'var(--muted)',marginTop:2}}>{finishData.type==='run'?'🏃 Corrida':'🚴 Bike'} · Rastro salvo no mapa</div></div>
